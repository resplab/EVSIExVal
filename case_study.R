...
EVSI(model, val_data[1:500,] , z=0.02, NULL, n_sim=settings$n_sim, prior=list(prev=c(1,1), sn=c(1,1), sp=c(1,1)))$EVPI
...
for(sample_size in settings$val_sample_size)
{
  ...
  for(z in zs) 
  {
    EVPI <- EVSI(model, dt , z, NULL, n_sim=settings$n_sim, prior=list(prev=c(1,1), sn=c(1,1), sp=c(1,1)))$EVPI
...
for(sample_size in settings$val_sample_size)
{
  ...
  {
    tmp <- EVSI(model, val_data[1:sample_size,], z, n_sim=settings$n_sim,  settings$future_sample_sizes)
...
for(z in settings$zs)
{
  ...
  for(sample_size in settings$val_sample_size)
  {
    ...
    par(las=2)
    plot(c(0,settings$future_sample_sizes), c(0,df[1,4:9]), type='l', ylim=c(0,max_y), xlab="Future sample size", ylab="EVSI", col=my_colors[i], xaxt="n")
...
plot(c(0,settings$future_sample_sizes), c(0,unlist(df[1,4:9]))/scale, type='l', ylim=c(0,max_y), xlab="Future sample size", ylab="EVSI", col='red')
#title(paste(df$val_size, z))
