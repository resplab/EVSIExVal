...
The rest of the proof is by contradiction. We show that two different CDFs $F_1$ and $F_2$ with the same $m$ cannot have the same $\int_0^1F^2(x)dx$. 
...
Let $z$ be the unique crossing point of the two CDFs, and let $z*=F_1(z)=F_2(z)$ be the CDF value at this point. We break $\int_0^1F^2(x)dx$ into two parts  around $z$:
...
Without loss of generality, assume we label $F$s such that $F_1(x) > F_2(x)$ when $x \in (0,z)$. In this region,  $F_1(x)-F_2(x) > 0$, and (due to Fs monotonically increasing) $0<F_1(x)+F_2(x) < F_1(z)+F_2(z)$. As such, replacing $F_1(x)+F_2(x)$ by the larger positive quantity $F_1(z)+F_2(z)=2z^*$ will increase this term. As well, in the $x\in (z,1)$ region, $F_1(x)-F_2(x) < 0$, and $0<F_1(x)+F_2(x) < F_1(z)+F_2(z)$. As such, replacing $F_1(x)+F_2(x)$ by the smaller positive quantity $2z^*$ will also increase this term. Therefore we have

$\int_0^1 (F_1^2(x)-F_2^2(x))dx < \int_{x \in (0,z)} (F_1(x)-F_2(x))2z^*dx+\int_{x \in (z,1)} (F_1(x)-F_2(x))2z^*dx$, and the term on the right hand side is zero because of the equality of the area under the CDFs. Therefore, $\int_0^1 (F_1^2(x)-F_2^2(x))dx < 0$, contradicting their equality.
